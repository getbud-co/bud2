...
using Xunit;
using GoalsPage = Bud.BlazorWasm.Features.Goals.Pages.Goals;

namespace Bud.BlazorWasm.Tests.Features.Goals;
...
public sealed class GoalsPageTests : TestContext
{
    ...
    public void Render_ShouldShowPageHeader()
    {
        ...
        cut.Markup.Should().Contain("Metas");
        cut.Markup.Should().Contain("Criar meta");
    }
    ...
    private IRenderedComponent<GoalsPage> RenderGoalsPage(string? goalsJson = null)
    {
        var authSessionJson = """
            {
              "Token":"token",
        ...
        return RenderComponent<GoalsPage>();
    }
    ...
    private sealed class SessionJsRuntime(string authSessionJson) : IJSRuntime
    {
        ...
        public ValueTask<TValue> InvokeAsync<TValue>(string identifier, CancellationToken cancellationToken, object?[]? args)
        {
            ...
            if (identifier == "localStorage.getItem")
            {
                if (args is { Length: > 0 } && string.Equals(args[0]?.ToString(), "bud.organization.selected", StringComparison.Ordinal))
                {
