...
using Xunit;
using GoalTemplatesPage = Bud.BlazorWasm.Features.Templates.Pages.GoalTemplates;

namespace Bud.BlazorWasm.Tests.Features.Templates;

public sealed class GoalTemplatesPageTests : TestContext
{
    [Fact]
    public void OpenCreateModal_ShouldShowFormModalWithTemplateTitle()
    {
        var cut = RenderGoalTemplatesPage();
        var instance = cut.Instance;

        InvokePrivateVoid(instance, "OpenCreateModal");
        cut.Render();

        cut.Markup.Should().Contain("Criar modelo");
        cut.Markup.Should().Contain("Nome do template");
    }
    ...
    private IRenderedComponent<GoalTemplatesPage> RenderGoalTemplatesPage()
    {
        ...
        return RenderComponent<GoalTemplatesPage>();
    }
    ...
    private sealed class SessionJsRuntime : IJSRuntime
    {
        ...
        public ValueTask<TValue> InvokeAsync<TValue>(string identifier, CancellationToken cancellationToken, object?[]? args)
        {
            if (identifier == "localStorage.getItem" &&
                args is { Length: > 0 } &&
                string.Equals(args[0]?.ToString(), "bud.organization.selected", StringComparison.Ordinal))
            {
