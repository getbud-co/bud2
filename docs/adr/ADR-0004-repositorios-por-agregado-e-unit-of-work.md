...
- Publicar contratos de repositório em `src/Api/Bud.Application/Features/<Feature>/` (ex: `IMissionRepository` em `Bud.Application.Features.Missions`), pois são os casos de uso quem definem o que precisam da persistência.
- Implementações residem em `src/Api/Bud.Infrastructure/Features/<Feature>/`.
