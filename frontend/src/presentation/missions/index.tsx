...
} from "@/types"; // MissionMember used in buildMissionFromForm
import {
  numVal,
  getOwnerName,
  getOwnerInitials,
...
import { TeamFilter } from "./components/Filters/components/TeamFilter";
import { StatusFilter } from "./components/Filters/components/StatusFilter";
import { OwnerFilter } from "./components/Filters/components/OwnerFilter";
import { ItemTypeFilter } from "./components/Filters/components/ItemTypeFilter";
import { IndicatorTypeFilter } from "./components/Filters/components/IndicatorTypeFilter";
import { ContributionFilter } from "./components/Filters/components/ContributionFilter";
import { SupporterFilter } from "./components/Filters/components/SupporterFilter";
import { TaskStateFilter } from "./components/Filters/components/TaskStateFilter";
import { MissionStatusFilter } from "./components/Filters/components/MissionStatusFilter";
import { PeriodFilter } from "./components/Filters/components/PeriodFilter";
...
export function MissionsPage({
  ...
  const [expandedMissionOverlayKey] = useState(0);
      ...
      setEditingItem({
        ...
        goalValue:
          goalType === "reach"
            ...
            : "",
        goalValueMin:
          goalType === "above" || goalType === "between"
            ? String(numVal(drawerIndicator.lowThreshold))
            : "",
        goalValueMax:
          goalType === "below" || goalType === "between"
  ...
  function saveDrawerEdit() {
    ...
    if (drawerMode === "indicator" && drawerIndicator) {
      const updated = {
        ...
        targetValue: String(
          Number(editingItem.goalValue) || numVal(drawerIndicator.targetValue),
        ),
        lowThreshold: String(
          Number(editingItem.goalValueMin) ||
            numVal(drawerIndicator.lowThreshold),
        ),
        highThreshold: String(
          Number(editingItem.goalValueMax) ||
      ...
      setMissions((prev) => {
        function updateInList(list: Mission[]): Mission[] {
          return list.map((m) => ({
            ...m,
            keyResults: (m.indicators ?? []).map((indicator) =>
    ...
    setMissions((prev) => {
      function updateTask(t: MissionTask): MissionTask {
        ...
        return {
          ...
          owner: task.owner ?? t.owner,
        };
      }
      function updateKR(kr: Indicator): Indicator {
        return {
          ...kr,
      ...
      function updateMission(m: Mission): Mission {
        return {
          ...
          indicators: m.indicators?.map(updateKR),
  ...
  const drawerIndicatorRef = useRef<Indicator | null>(null);
    ...
    setMissions((prev) => {
      function updateKR(kr: Indicator): Indicator {
        ...
        const merged: MissionTask[] = tasks.map((dt) => {
          ...
          const ownerUser = dt.ownerId
            ? users.find((u) => u.id === dt.ownerId)
            : null;
          const ownerJoin = ownerUser
            ? {
                id: ownerUser.id,
                fullName: ownerUser.fullName,
                initials: ownerUser.initials ?? null,
              }
            : undefined;
          if (existing) {
            return {
              ...
              title: dt.title,
              ownerId: dt.ownerId,
              owner: ownerJoin ?? existing.owner,
            };
          }
  ...
  function handleConfirmCheckin() {
    ...
    const checkInAuthorName = checkInAuthor.label;
    ...
    setMissions((prev) => {
      function updateIndicators(krs: Indicator[]): Indicator[] {
        return krs.map((kr) => {
          const nextChildren = kr.children
            ? updateIndicators(kr.children)
            : undefined;
      ...
      function recalcMission(mission: Mission): Mission {
        ...
        const progressSources = [
          ...nextIndicators.map((kr) => kr.progress),
  ...
  function handleUpdateDrawerCheckIn(
    ...
    // TODO: Create real update check-in API call
    if (true) {
  ...
  function handleDeleteDrawerCheckIn(checkInId: string) {
    // TODO: Create real delete check-in API call
  ...
  const displayedMissions = useMemo(() => {
    ...
    function missionHasContribution(mission: Mission): boolean {
      if (
        ...
        return true;
      if ((mission.keyResults ?? []).some((kr) => keyResultHasContribution(kr)))
        return true;
      return false;
    }
    ...
    function filterMissionNode(mission: Mission): Mission | null {
      ...
      const nextIndicators = (mission.keyResults ?? [])
  ...
  const activeTeamIds = selectedTeams
    .filter((id) => id !== "all")
    .map(resolveTeamId);
    ...
    for (const m of displayedMissions) {
      ...
      if (!groups.has(key)) {
        const teamOpt = matchedTeamId
          ? teamFilterOptions.find((t) => t.id === matchedTeamId)
          : null;
        groups.set(key, {
  ...
  function materializeMissionItems(
    ...
    ownerId: string,
  ): { keyResults: Indicator[]; children: Mission[] } {
    const keyResults: Indicator[] = [];
  ...
  function buildMissionFromForm(existing?: Mission): Mission {
    ...
    const progress =
      tree.keyResults.length > 0
        ? Math.round(
            tree.keyResults.reduce((acc, kr) => acc + kr.progress, 0) /
    ...
    return (
      <div className={styles.addItemCard}>
        <div className={styles.addItemCardHeader}>
          <span className={styles.addItemCardTitle}>
            {isEditingExisting
          ...
          {editingItem.measurementMode === "manual" &&
            editingItem.manualType && (
              <div className={styles.goalSection}>
                ...
                {editingItem.manualType === "survey" &&
                  editingItem.surveyId && (
                    <div className={styles.goalRow}>
              ...
              return (
                ...
                >
                  ...
                  <div className={styles.measureMenuText}>
                    <span className={styles.measureMenuLabel}>
                      {mode.label}
                    </span>
                    <span className={styles.measureMenuDesc}>
                      {mode.description}
                    </span>
                  </div>
                  {mode.id === "manual" && (
  ...
  function renderReviewItems(items: MissionItemData[], depth: number) {
    return items.map((item) => {
      ...
      return (
        <div
          key={item.id}
          className={styles.reviewItemGroup}
          style={
            depth > 0
        ...
        {items.map((item) => {
          ...
          if (isBeingEdited) {
            return (
              <div
                key={item.id}
                className={isNested ? undefined : styles.addedItemWrapper}
              >
                {isNested ? (
          ...
          return (
            <div
              ...
              className={
                isNested
                  ? styles.subMissionItemWrapper
                  : styles.addedItemWrapper
              }
            >
                ...
                <div className={styles.addedItemClickable} onClick={handleEdit}>
                  <div className={styles.addedItemInfo}>
                    ...
                    {canHaveChildren && childCount > 0 && (
                      <span className={styles.addedItemCount}>
                  ...
                  <div className={styles.addedItemMeta}>
                    ...
                    {goalSummary && (
                      <span className={styles.addedItemGoal}>
                        {goalSummary}
                      </span>
                    )}
                  </div>
      ...
      >
        <CardBody>
          ...
          <div className={styles.cardGridFooter}>
            ...
            {/* eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions */}
            <div
              className={styles.cardGridActions}
  ...
  return (
    <div className={styles.page}>
          ...
          <Card padding="sm">
            <CardBody>
              <FilterBar
                ...
                onClearAll={
                  activeFilters.length > 0 ? handleClearAll : undefined
                }
                onSaveView={
                  activeFilters.length > 0 ? handleOpenSaveModal : undefined
                }
                saveViewLabel={
              ...
              <TeamFilter
                ...
                onChange={checklistFilterOnChange(setSelectedTeams)}
              />
              ...
              <OwnerFilter
                ...
                onClose={() => setOpenFilter(null)}
                onChange={checklistFilterOnChange(setSelectedOwners)}
              />
              ...
              <ItemTypeFilter
                ...
                onClose={() => setOpenFilter(null)}
                onChange={checklistFilterOnChange(setSelectedItemTypes)}
              />
              ...
              <IndicatorTypeFilter
                ...
                onClose={() => setOpenFilter(null)}
                onChange={checklistFilterOnChange(setSelectedIndicatorTypes)}
              />
              ...
              <ContributionFilter
                ...
                onClose={() => setOpenFilter(null)}
                onChange={checklistFilterOnChange(setSelectedContributions)}
              />
              ...
              <SupporterFilter
                ...
                onClose={() => setOpenFilter(null)}
                onChange={checklistFilterOnChange(setSelectedSupporters)}
              />
              ...
              <MissionStatusFilter
                ...
                onClose={() => setOpenFilter(null)}
                onChange={checklistFilterOnChange(setSelectedMissionStatuses)}
              />
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={styles.kanbanColumnBody}>
                                ...
                                >
                                  <CardBody>
                                    <div className={styles.kanbanCardRow}>
                                      {item.type === "task" && (
                                        <span
                                          className={styles.kanbanTaskCheck}
                                          onClick={(e) => e.stopPropagation()}
                                        >
                                          <Checkbox
                                      ...
                                      {item.type !== "task" && Icon && (
                                        <Icon
                                          size={20}
                                          className={styles.kanbanCardIcon}
                                        />
                                      )}
                                      ...
                                      <div className={styles.kanbanCardInfo}>
                                        <span
                                          className={`${styles.kanbanCardTitle} ${item.type === "task" && item.done ? styles.taskNameDone : ""}`}
                                        >
                                          {item.label}
                                        </span>
                                        <span
                                          className={styles.kanbanCardMission}
                                        >
                                          {item.missionTitle}
                                        </span>
                                        {isMultiTeam && item.teamName && (
                                          <span
                                            className={styles.kanbanTeamTag}
                                          >
                                            <span
                                              className={styles.teamDot}
                                              style={{
                                    ...
                                    {item.type === "task" ? (
                                      ...
                                      <>
                                        {/* eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions */}
                                        <div
                                          className={styles.kanbanCardProgress}
                                          onClick={(e) => e.stopPropagation()}
                                        >
                                          <GoalProgressBar
                                            label=""
                                    ...
                                    <div className={styles.kanbanCardFooter}>
                                      <div className={styles.kanbanCardMeta}>
                                        ...
                                        {item.period && (
                                          <span
                                            className={styles.kanbanCardPeriod}
                                          >
                                            {item.period}
                                          </span>
                                        )}
                                      ...
                                      >
                                        <Button
                                          ref={(
                                            ...
                                            kanbanMoveBtnRefs.current[item.id] =
                                              el;
                                          }}
                                          variant="tertiary"
                                          size="sm"
                                        ...
                                        >
                                          ...
                                          >
                                            <span
                                              className={styles.kanbanMoveLabel}
                                            >
                                              Mover para
                                            </span>
                                      ...
                                      <div className={styles.kanbanChildren}>
                                        {item.children!.map((child) => {
                                          ...
                                          return (
                                            <div
                                              key={child.id}
                                              className={styles.kanbanChildItem}
                                            >
                                              {ChildIcon && (
                                                <ChildIcon
                                                  size={14}
                                                  className={
                                                    styles.kanbanChildIcon
                                                  }
                                                />
                                              )}
                                              <div
                                                className={
                                                  styles.kanbanChildInfo
                                                }
                                              >
                                                <span
                                                  className={
                                                    styles.kanbanChildLabel
                                                  }
                                                >
                                                  {child.label}
                                                </span>
                          ...
                          {colItems.length === 0 && (
                            <span className={styles.kanbanEmpty}>
                              Nenhum item
                            </span>
                          )}
                        </div>
                ...
                {activeFilters.map((filterId) => {
                  ...
                  return (
                    <li key={filterId} className={styles.saveFilterItem}>
                      {Icon && (
                        <Icon size={14} className={styles.saveFilterIcon} />
                      )}
                      <span className={styles.saveFilterName}>
                        {filterMeta?.label ?? filterId}
                      </span>
                      <span className={styles.saveFilterValue}>
      ...
      >
        ...
        <ModalBody>
          ...
          >
            ...
            <strong>{removeContribConfirm?.targetMissionTitle}</strong>?
          </p>
        </ModalBody>
        <ModalFooter>
          <Button
            variant="secondary"
            size="md"
            onClick={() => setRemoveContribConfirm(null)}
          >
            Cancelar
          </Button>
          <Button
            ...
            onClick={() => {
              ...
              handleRemoveContribution(
                removeContribConfirm.itemId,
                removeContribConfirm.itemType,
                removeContribConfirm.targetMissionId,
              );
      ...
      <MissionDetailsDrawer
        ...
        onOpenTaskFromIndicator={(taskId: string) => {
          const result = findTaskById(taskId, missions);
          if (result) handleOpenTaskDrawer(result.task, result.parentLabel);
        }}
        onChangeTaskOwner={(ownerId: string) => {
          ...
          const ownerJoin = ownerUser
            ...
            : undefined;
          setDrawerTask((prev) =>
            prev ? { ...prev, ownerId, owner: ownerJoin ?? prev.owner } : prev,
          );
        }}
      ...
      >
              ...
              return (
                <div className={styles.deleteText}>
                  <p>
                    ...
                    <strong>{deleteMissionTarget.title}</strong>?
                  </p>
                  {totalItems > 0 && (
                    <Alert
                      variant="warning"
                      title="Todos os itens serão removidos"
                    >
                      {krs.length > 0 && (
                        <span>
                          {krs.length}{" "}
        ...
        <ModalFooter>
          <Button
            variant="secondary"
            size="md"
            onClick={() => setDeleteMissionTarget(null)}
          >
            Cancelar
          </Button>
          <Button
                    ...
                    if (content.includes(ex.objective)) {
                      ...
                      setNewMissionItems(
                        ex.keyResults.map((kr) => {
                          const parsed = parseIndicatorGoal(kr);
                          return {
                            id: generateItemId(),
                            name: kr,
                            description: "",
        ...
        <ModalBody>
          ...
          {createStep === 1 &&
            (() => {
              ...
              return (
                <div className={styles.createBody}>
                  <p className={styles.createStepTitle}>
                  ...
                  <PopoverSelect
                    ...
                    onChange={(val) => {
                      setSelectedMissionOwners(val ? [val] : []);
                    ...
                    {selectedTags.length > 0 && (
                      <div className={styles.reviewRow}>
                        ...
                        <div className={styles.reviewTags}>
                          {selectedTags.map((id) => {
                            const label =
                              missionTagOptions.find((t) => t.id === id)
                                ?.label ??
