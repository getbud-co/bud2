...
function getMissionHealth(mission: {
  ...
  progress: number;
  dueDate: string | null;
}): "at-risk" | "healthy" | "needs-attention" {
...
export function useBriefingReadModel(): BriefingItem[] {
  ...
  return useMemo(() => {
    ...
    const atRiskMissions = activeMissions.filter(
      (m) => getMissionHealth(m) === "at-risk",
    );
    if (atRiskMissions.length > 0) {
      ...
      const worstMission = atRiskMissions.find(
        (m) => m.progress === lowestProgress,
      );
