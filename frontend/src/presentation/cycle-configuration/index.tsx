...
export function CyclesComponent() {
  ...
  function openEdit(cycle: Cycle) {
    ...
    setFormDates([
      isoToCalendarDate(cycle.startDate),
      isoToCalendarDate(cycle.endDate),
    ]);
  ...
  return (
    <>
      <Alert variant="info" title="Defina os períodos da sua organização">
        Os ciclos criados aqui ficam disponíveis como períodos pré-definidos nas
        funcionalidades de missões e pesquisas da plataforma.
      </Alert>
                    ...
                    <RowActionsPopover
                      ...
                      onToggle={() =>
                        setActionsPopoverCycle(
                          actionsPopoverCycle === c.id ? null : c.id,
                        )
                      }
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} size="md">
        ...
        <ModalBody>
          <div className="flex flex-col gap-[var(--sp-md)]">
            ...
            <div>
              ...
              <ChoiceBoxGroup
                value={formStatus}
                onChange={(v) => setFormStatus(v as CycleStatus)}
              >
                <ChoiceBox
                  value="active"
                  title="Ativo"
        ...
        <ModalFooter>
          ...
          <Button
            variant="primary"
            size="md"
            disabled={!formName.trim() || !formDates[0] || !formDates[1]}
            onClick={handleSave}
          >
        ...
        <ModalBody>
          {deletingCycle && (
            <p className="[font-family:var(--font-body)] [font-size:var(--text-sm)] text-[var(--color-neutral-700)] m-0 leading-[1.5]">
              ...
              <strong>{deletingCycle.name}</strong>? Missões e indicadores
              vinculados a este período não serão afetados.
            </p>
          )}
        </ModalBody>
        <ModalFooter>
