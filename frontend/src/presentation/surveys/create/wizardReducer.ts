...
export type WizardAction =
  ...
  | {
      ...
      payload: {
        ...
        config: Partial<WizardPerspective>;
      };
    }
  ...
  | {
      ...
      payload: { id: string; changes: Partial<WizardQuestion> };
    }
  | { type: "REORDER_QUESTIONS"; payload: WizardQuestion[] }
  | {
      type: "REORDER_ALL";
      payload: { sections: WizardSection[]; questions: WizardQuestion[] };
    }
  | { type: "ADD_SECTION"; payload: WizardSection }
  | {
      type: "UPDATE_SECTION";
      payload: { id: string; changes: Partial<WizardSection> };
    }
  ...
  | {
      ...
      payload: { channel: "inApp" | "email" | "slack"; enabled: boolean };
    }
...
const DEFAULT_PERSPECTIVES: WizardPerspective[] = [
  {
    ...
    enabled: true,
    isAnonymous: false,
    peerSelectionMethod: "manager_assigns",
    minEvaluators: 1,
    maxEvaluators: 1,
  },
  {
...
export const initialWizardState: SurveyWizardState = {
  ...
  notifyManagers: true,
  managerNotificationTemplate:
    'Olá! A pesquisa "{nome}" está em andamento. Acompanhe a adesão do seu time e incentive a participação.',
  nonRespondentNotificationTemplate:
    'Olá! Você ainda não respondeu a pesquisa "{nome}". Sua participação é muito importante para nós.',
  deliveryInApp: true,
  deliveryEmail: true,
  deliverySlack: false,
  startDate: null,
...
function loadTemplateQuestions(surveyType: SurveyType): {
  sections: WizardSection[];
  questions: WizardQuestion[];
} {
  ...
  switch (action.type) {
    case "SELECT_TEMPLATE": {
      ...
      return {
        ...
        perspectives: category === "ciclo" ? DEFAULT_PERSPECTIVES : [],
      };
    }
    ...
    case "SET_METADATA":
      return {
        ...
        cycleId:
          action.payload.cycleId === undefined
            ? state.cycleId
    ...
    case "SET_SCOPE_TYPE":
      return {
        ...
        scope: {
          ...state.scope,
          scopeType: action.payload,
          teamIds: action.payload === "company" ? [] : state.scope.teamIds,
        },
      };
    ...
    case "ADD_TEAM":
      return {
        ...
        scope: {
          ...state.scope,
          teamIds: [...state.scope.teamIds, action.payload],
        },
      };
    ...
    case "REMOVE_TEAM":
      return {
        ...
        scope: {
          ...state.scope,
          teamIds: state.scope.teamIds.filter((id) => id !== action.payload),
        },
      };
    ...
    case "SET_PERSPECTIVE_CONFIG":
      return {
        ...
        perspectives: state.perspectives.map((p) =>
          p.perspective === action.payload.perspective
            ? { ...p, ...action.payload.config }
            : p,
        ),
      };
