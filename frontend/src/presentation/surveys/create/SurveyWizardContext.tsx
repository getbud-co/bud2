...
import type { SurveyWizardState, WizardStep } from "@/types/survey/survey";
...
function validateStep(state: SurveyWizardState): boolean {
  switch (state.step) {
    ...
    case 1:
      if (state.scope.scopeType === "team" && state.scope.teamIds.length === 0)
        return false;
      return true;
    case 2:
    ...
    case 3:
      return (
        state.name.trim().length > 0 && !!state.startDate && !!state.endDate
      );
    case 4:
      if (state.launchOption === "scheduled" && !state.scheduledLaunchAt)
        return false;
      return true;
    default:
      return false;
...
export function useWizard(): WizardContextValue {
  ...
  if (!ctx)
    throw new Error("useWizard must be used within SurveyWizardProvider");
  return ctx;
}
