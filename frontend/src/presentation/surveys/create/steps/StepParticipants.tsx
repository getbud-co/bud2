...
import type { ScopeType, EvaluationPerspective } from "@/types/survey/survey";
...
export function StepParticipants() {
  ...
  const { users, teamOptions, resolveTeamId, resolveUserId, getTeamNameById } =
    usePeopleData();
  ...
  const selectedScopeTeamIds = useMemo(
    () =>
      Array.from(
        new Set(state.scope.teamIds.map((teamId) => resolveTeamId(teamId))),
      ),
  ...
  const scopedPeople = useMemo(() => {
    ...
    if (state.scope.scopeType === "team") {
      ...
      return allPeople.filter((person) =>
        person.teamIds.some((teamId) => selectedTeamSet.has(teamId)),
      );
    }
    if (
      state.scope.scopeType === "individual" &&
      state.scope.userIds.length > 0
    ) {
      const scopedUserIds = new Set(
        state.scope.userIds.map((userId) => resolveUserId(userId)),
      );
  ...
  const uniqueTeamIds = useMemo(
    () =>
      Array.from(
        new Set(scopedPeople.flatMap((person) => person.teamIds)),
      ).sort((a, b) => {
  ...
  const allVisibleSelected =
    ...
    filteredPeople.every((person) => !excludedUserIdSet.has(person.id));
  ...
  function handleToggleAll() {
    if (allVisibleSelected) {
      ...
      dispatch({
        ...
        payload: Array.from(newExcluded),
      });
    } else {
      const visibleIds = new Set(filteredPeople.map((person) => person.id));
      dispatch({
        type: "SET_EXCLUDED_USERS",
        payload: Array.from(excludedUserIdSet).filter(
  ...
  function togglePerspective(perspective: EvaluationPerspective) {
    const current = state.perspectives.find(
  ...
  return (
    <div className={styles.step}>
      ...
      <section className={styles.section}>
        <h3 className={styles.sectionTitle}>Parte 1: Quem será avaliado?</h3>
          ...
          <Select
            ...
            onChange={(val: string | string[]) => {
              const newIds = Array.from(
                new Set(
                  (Array.isArray(val) ? val : [val]).map((teamId) =>
        ...
        <div className={styles.peopleCard}>
          <div className={styles.peopleHeader}>
            <div className={styles.peopleHeaderTop}>
              <div className={styles.peopleHeaderLeft}>
                <Checkbox
                  ...
                  indeterminate={
                    !allVisibleSelected &&
                    excludedInScope.length < scopedPeople.length
                  }
                  onChange={handleToggleAll}
                />
                <span className={styles.peopleCount}>
                  <strong>{selectedCount}</strong> de {scopedPeople.length}{" "}
                  selecionados
                </span>
                {excludedInScope.length > 0 && (
                  <Badge color="orange" size="sm">
                    {excludedInScope.length} removido
                    {excludedInScope.length > 1 ? "s" : ""}
                  </Badge>
                )}
              </div>
            ...
            <PopoverSelect
              ...
              value={filterStatus}
              onChange={(id) =>
                setFilterStatus(id as "all" | "included" | "excluded")
              }
            />
          </div>
          ...
          <div className={styles.peopleList}>
            {filteredPeople.map((person) => {
              ...
              const teamText =
                person.teamLabels.length > 0
                  ? person.teamLabels.join(", ")
                  : "Sem time";
              return (
                <label
                  key={person.id}
        ...
        <>
          ...
          <section className={styles.section}>
            ...
            <div className={styles.perspectivesGrid}>
              {PERSPECTIVES.map((p) => {
                const persp = state.perspectives.find(
                  (sp) => sp.perspective === p.key,
                );
                return (
                  <Checkbox
                    key={p.key}
      ...
      <section className={styles.section}>
        ...
        <div className={styles.toggleList}>
          <Toggle
            ...
            onChange={() =>
              dispatch({ type: "TOGGLE_EXCLUSION", payload: "trialPeriod" })
            }
          />
          <Toggle
            ...
            onChange={() =>
              dispatch({ type: "TOGGLE_EXCLUSION", payload: "leave" })
            }
          />
          <Toggle
            ...
            onChange={() =>
              dispatch({ type: "TOGGLE_EXCLUSION", payload: "vacation" })
            }
          />
        </div>
      </section>
      ...
      {showLgpdWarning && (
        <Alert variant="warning" title="Grupo pequeno para anonimato">
          ...
          {state.lgpdMinGroupSize} participantes. O grupo atual tem ~
          {participantCount}.
        </Alert>
      )}
    </div>
