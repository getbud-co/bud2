...
import type { HeatmapCell, FilterOption } from "@mdonangelo/bud-ds";
import {
  Warning,
  TrendUp,
  TrendDown,
...
const PRIORITY_CONFIG: Record<
  string,
  { label: string; color: "error" | "warning" | "neutral" }
> = {
...
export function OverviewTab({ data }: OverviewTabProps) {
  ...
  const filteredHeatmapData = useMemo<HeatmapCell[]>(() => {
    ...
    const entries = !hasDeptFilter
      ...
      : review.heatmap.entries.filter((e) =>
          selectedDepartments.has(e.department),
        );
  ...
  const filteredAttentionPoints = useMemo(() => {
    ...
    return !hasDeptFilter
      ...
      : review.attentionPoints.filter((ap) =>
          selectedDepartments.has(ap.department),
        );
  }, [review, selectedDepartments, hasDeptFilter]);
  ...
  const filteredActionItems = useMemo(() => {
    ...
    return !hasDeptFilter
      ? review.actionItems
      : review.actionItems.filter(
          (ai) =>
            selectedDepartments.has(ai.department) || ai.department === "Geral",
        );
  }, [review, selectedDepartments, hasDeptFilter]);
    ...
    <div className={styles.tab}>
      ...
      <Card padding="md">
        <CardHeader
          ...
          description="Média das respostas numéricas por departamento e pergunta"
        />
        <CardBody>
          <Heatmap
          ...
          <CardHeader
            ...
            action={
              <Badge color="error" size="sm">
                {filteredAttentionPoints.length}
              </Badge>
            }
          />
          <CardBody>
                ...
                <div key={ap.id} className={styles.attentionItem}>
                  ...
                  <div className={styles.attentionContent}>
                    <div className={styles.attentionHeader}>
                      <span className={styles.attentionQuestion}>
                        {ap.questionText}
                      </span>
                      <div className={styles.attentionScores}>
                        <Badge
                          color={
                            ap.severity === "critical" ? "error" : "warning"
                          }
                          size="sm"
                        >
      ...
      {review.trends.length > 0 && (
        <Card padding="md">
          <CardHeader
            title="Tendências"
            description="Evolução comparada ao período anterior"
          />
          <CardBody>
            ...
            <TableContent>
              ...
              <TableBody>
                {filteredActionItems.map((item) => {
                  ...
                  const status = ACTION_STATUS_CONFIG[item.status] ?? {
                    label: "Pendente",
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <Badge color={prio.color} size="sm">
                          {prio.label}
                        </Badge>
                      </TableCell>
