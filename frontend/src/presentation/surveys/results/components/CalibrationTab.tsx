...
import type { CalendarDate, FilterOption } from "@mdonangelo/bud-ds";
import {
  MagnifyingGlass,
  Export,
  Warning,
...
import type { SurveyResultData, CalibrationParticipant } from "../types";
...
const SESSION_STATUS_CONFIG: Record<
  string,
  { label: string; color: "caramel" | "warning" | "success" }
> = {
    ...
    return (
      <div className={styles.tab}>
        <Alert variant="info" title="Sem dados">
          Dados de calibragem não disponíveis para esta pesquisa.
        </Alert>
      </div>
    );
  }
...
const NINE_BOX_CELLS: NineBoxCellConfig[] = [
  ...
  // col 0 = baixo desempenho (left), col 2 = alto desempenho (right)
  {
    ...
    key: "0-0",
    label: "Enigma",
    baseColor: "var(--color-warning-200)",
    baseColorLight: "var(--color-warning-50)",
  },
  {
    row: 0,
    col: 1,
    ...
    baseColorLight: "var(--color-green-50)",
  },
  {
    row: 0,
    col: 2,
  ...
  },
  {
    ...
    key: "1-0",
    label: "Questionável",
    baseColor: "var(--color-error-200)",
    baseColorLight: "var(--color-error-50)",
  },
  {
    row: 1,
    col: 1,
    key: "1-1",
    label: "Mantenedor",
    baseColor: "var(--color-caramel-200)",
    baseColorLight: "var(--color-caramel-50)",
  },
  {
    row: 1,
    col: 2,
...
function NineBoxGrid({
  ...
  const { cells, maxCount } = useMemo(() => {
    ...
    for (const cell of map.values()) {
      cell.percent =
        total > 0 ? Math.round((cell.all.length / total) * 100) : 0;
              ...
              {NINE_BOX_CELLS.map((cellConfig) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      onCellClick(isActive ? null : cellConfig.key)
                    }
                    aria-label={`${cellConfig.label}: ${count} colaboradores, ${data.calibrated} calibrados`}
                    aria-pressed={isActive}
                  >
                    <span className={styles.nineBoxCellLabel}>
                      {cellConfig.label}
                    </span>
                    ...
                    {count > 0 ? (
                      <>
                        ...
                        <span className={styles.nineBoxPercent}>
                          {data.percent}%
                        </span>
                        ...
                        <div className={styles.nineBoxProgress}>
                          <div
                            ...
                            style={{
                              width: `${count > 0 ? (data.calibrated / count) * 100 : 0}%`,
                            }}
                          />
                        </div>
...
function CycleCalibration({ data }: { data: SurveyResultData }) {
  ...
  const [modalParticipant, setModalParticipant] =
    useState<CalibrationParticipant | null>(null);

  /* ——— Derived filter options ——— */
  const departmentOptions = useMemo(() => {
    const deps = [
      ...new Set(calibration.participants.map((p) => p.department)),
    ].sort();
  ...
  function getFilterLabel(id: string): string {
    switch (id) {
      ...
      case "potential": {
        ...
        return selectedPotential === "all"
          ? "Potencial"
          : `Potencial: ${opt?.label ?? ""}`;
      }
      case "bias": {
        const opt = BIAS_OPTIONS.find((o) => o.id === selectedBias);
        return selectedBias === "all"
          ? "Alerta de viés"
          : `Viés: ${opt?.label ?? ""}`;
      }
      case "period": {
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedPeriod[0] && selectedPeriod[1]) {
      ...
      items = items.filter(
        (p) => p.respondedAt >= startIso && p.respondedAt <= endIso,
      );
    }
    return items;
  }, [
  ...
  /* KPIs */
  const pendingCount =
    calibration.totalParticipants - calibration.calibratedCount;
  const completionRate = Math.round(
    (calibration.calibratedCount / calibration.totalParticipants) * 100,
  );

  /* Session status */
  const sessionConfig = SESSION_STATUS_CONFIG[calibration.sessionStatus] ?? {
    label: "Rascunho",
    color: "caramel" as const,
  };
  ...
  function handleSaveCalibration(
    ...
    toast.success(`Calibragem de ${participant.name} salva com sucesso`);
    ...
    <div className={styles.tab}>
      ...
      <div className={styles.sessionBar}>
        <div className={styles.sessionLeft}>
          <Badge color={sessionConfig.color} size="md">
            {sessionConfig.label}
          </Badge>
          <span className={styles.sessionText}>
            {calibration.calibratedCount}/{calibration.totalParticipants}{" "}
            calibrados
          </span>
        </div>
      ...
      {/* Filters */}
      <FilterBar
        filters={CALIBRATION_FILTER_OPTIONS.filter(
      ...
      <div className={styles.kpiGrid}>
        {[
          {
            label: "Participantes",
            value: calibration.totalParticipants,
            icon: Users,
            bg: "var(--color-caramel-100)",
            iconColor: "var(--color-caramel-600)",
          },
          {
            label: "Calibrados",
            value: calibration.calibratedCount,
            icon: CheckCircle,
        ...
        <TableContent>
          <TableHead>
            <TableRow>
              ...
              <TableHeaderCell
                sortable
                sortDirection={getSortDirection("selfScore")}
                onSort={() => handleSort("selfScore")}
              >
                Auto
              </TableHeaderCell>
              <TableHeaderCell
                sortable
                sortDirection={getSortDirection("managerScore")}
                onSort={() => handleSort("managerScore")}
              >
                Gestor
              </TableHeaderCell>
              <TableHeaderCell
                sortable
                sortDirection={getSortDirection("finalScore")}
                onSort={() => handleSort("finalScore")}
              >
                Score Final
              </TableHeaderCell>
            ...
            {sorted.map((p) => (
              <TableRow key={p.id} rowId={p.id}>
                ...
                <TableCell>
                  <span
                    style={{
                      color: getScoreColor(p.selfScore),
                      fontWeight: 600,
                    }}
                  >
                    {p.selfScore.toFixed(1)}
                  </span>
                </TableCell>
                <TableCell>
                  <span
                    style={{
                      color: getScoreColor(p.managerScore),
                      fontWeight: 600,
                    }}
                  >
                    {p.managerScore.toFixed(1)}
                  </span>
                </TableCell>
                <TableCell>
                  {p.finalScore != null ? (
                    <span
                      style={{
                        color: getScoreColor(p.finalScore),
                        fontWeight: 700,
                ...
                <TableCell>
                  <Badge
                    color={p.status === "calibrado" ? "success" : "warning"}
                    size="sm"
                  >
