...
import type {
  SurveyListItemData as SurveyListItem,
...
function templateRecordToPreviewTemplate(
  ...
  const unsectioned = template.questions.filter(
...
export function SurveysPage() {
  ...
  const respondentState = location.state as {
    filterRespondentId?: string;
    filterRespondentName?: string;
  } | null;
  ...
  function selectRespondent(id: string, name: string) {
    navigate(location.pathname, {
      replace: true,
      state: { filterRespondentId: id, filterRespondentName: name },
    });
  ...
  const currentView = viewId
    ? views.find((v) => v.id === viewId && v.module === "surveys")
    : null;
  ...
  const [createSelectedTemplateId, setCreateSelectedTemplateId] = useState<
    string | null
  >(null);
  ...
  const previewTemplate = useMemo<SurveyTemplate | null>(() => {
    ...
    const template = createTemplates.find(
    ...
    switch (id) {
      case "status":
        return selectedStatus === "all"
          ? "Status"
          : `Status: ${STATUS_OPTIONS.find((o) => o.id === selectedStatus)?.label}`;
      case "type":
        return selectedType === "all"
          ? "Tipo"
          : `Tipo: ${TYPE_OPTIONS.find((o) => o.id === selectedType)?.label}`;
      case "category":
        return selectedCategory === "all"
          ? "Categoria"
          : `Categoria: ${CATEGORY_OPTIONS.find((o) => o.id === selectedCategory)?.label}`;
      case "period": {
        const [s, e] = selectedPeriod;
        if (s && e)
          return `Período: ${formatCalendarDate(s)} — ${formatCalendarDate(e)}`;
        return "Período";
      }
      default:
        return id;
    ...
    if (sortKey) {
      ...
      list.sort((a, b) => {
        switch (sortKey) {
          ...
          case "startDate":
            return dir * (parseDate(a.startDate) - parseDate(b.startDate));
          default:
            return 0;
        }
      });
  ...
  const avgResponseRate = useMemo(() => {
    const withResponses = filtered.filter(
      (s) => s.status === "active" || s.status === "closed",
    );
    if (withResponses.length === 0) return 0;
    return Math.round(
      withResponses.reduce((sum, s) => sum + s.completionRate, 0) /
        withResponses.length,
    );
  }, [filtered]);
  const pendingSurveys = filtered.filter(
    (s) => s.status === "scheduled" || s.status === "draft",
  ).length;
  ...
  function handleResume(survey: SurveyListItem) {
    setSurveys((prev) =>
      prev.map((s) =>
        s.id === survey.id ? { ...s, status: "active" as SurveyStatus } : s,
      ),
    );
    ...
    items.push(
      {
        ...
        onClick: () =>
          navigate(`/surveys/new/participantes?${editQuery.toString()}`),
      },
      {
        id: "duplicate",
        label: "Duplicar",
  ...
  function handleNext() {
    ...
    const template = createTemplates.find(
      (t) => t.id === createSelectedTemplateId,
    );
    if (!template) return;
  ...
  return (
    <div className={styles.page}>
      ...
      <Card padding="sm">
        <CardBody>
          <FilterBar
            ...
            onClearAll={
              activeFilters.length > 0 || !!filterRespondentId
                ? handleClearAll
                : undefined
            }
            onSaveView={
            ...
            <div className={styles.respondentDropdownBody}>
              {peopleOptions.map((person) => (
                ...
                >
                  ...
                  {filterRespondentId === person.id && (
                    <Check size={14} className={styles.filterDropdownCheck} />
                  )}
                </button>
              ))}
            </div>
            ...
            <Card padding="sm">
              <CardBody>
                <div className={styles.indicatorCard}>
                  <div
                    className={styles.indicatorIconWrapper}
                    style={{
            ...
            <TableContent>
              <TableHead>
                <TableRow>
                  ...
                  <TableHeaderCell
                    sortable
                    sortDirection={getSortDirection("completionRate")}
                    onSort={() => handleSort("completionRate")}
                  >
                    Respostas
                  </TableHeaderCell>
                    ...
                    <TableRow key={s.id} rowId={s.id}>
                      ...
                      <TableCell>
                        <div className={styles.typeBadge}>
                          <Badge
                            color={s.category === "ciclo" ? "wine" : "orange"}
                            size="sm"
                          >
                            {templateNameByType.get(s.type) ??
                              getTypeLabel(s.type)}
                          </Badge>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge color={statusConf.color} size="sm">
                          {statusConf.label}
                        </Badge>
                      </TableCell>
                      <TableCell>
      ...
      <Modal
        open={!!pauseSurvey}
        onClose={() => setPauseSurvey(null)}
        size="sm"
      >
        <ModalHeader
          title="Pausar pesquisa"
          onClose={() => setPauseSurvey(null)}
        />
        <ModalBody>
          {pauseSurvey && (
      ...
      >
        ...
        <ModalBody>
          <div className={styles.createBody}>
            ...
            <p className={styles.createTitle}>
              Escolha o seu template de pesquisa
            </p>

            {createTemplates.length === 0 && (
              <Alert variant="info" title="Sem templates disponíveis">
                Vá em Configurações &gt; Templates de pesquisa para criar seu
                primeiro template.
              </Alert>
            )}
        ...
        <ModalFooter align="between">
          <Button variant="tertiary" size="md" onClick={resetCreate}>
            Cancelar
          </Button>
          <Button
            variant="primary"
            size="md"
            disabled={!createSelectedTemplateId}
            onClick={handleNext}
          >
            Próximo
          </Button>
