...
/**
 * Generate deterministic check-in dates for a given Indicator
 * Uses the Indicator ID as seed to ensure same dates on every access
 *
 * @param indicatorId The Key Result ID (used as seed for determinism)
 * @param count Number of check-ins to generate (default 4)
 * @param maxWeeksBack How far back to go (default 4 weeks)
 */
export function generateCheckInsForIndicator(
  indicatorId: string,
  count: number = 4,
  ...
  const now = today();
  const weekStart = startOfWeek(now);
  const hash = hashString(indicatorId);
 ...
 */
export function deterministicProgress(
  seed: string,
  min: number = 20,
  max: number = 95,
): number {
 ...
 */
export function deterministicCreatedAt(
  entityId: string,
  cycleStart: Date,
): string {
