...
function createIndicator(
  ownerId: string,
  status: "on_track" | "attention" | "off_track" | "completed" = "on_track",
) {
  return {
    id: `indicator-${Math.random().toString(36).slice(2, 7)}`,
    orgId: "org-1",
    missionId: "mission-1",
    parentKrId: null,
    ownerId,
...
describe("getUserMissionStats()", () => {
  ...
  it("counts owned key results", () => {
    const missions = [
      createMission({
        ...
        ownerId: "user-2",
        indicators: [
          createIndicator("user-1"),
          createIndicator("user-1"),
          createIndicator("user-2"),
        ],
      }),
    ];
    const result = getUserMissionStats(missions, "user-1");
    expect(result.ownedIndicators).toBe(2);
  });
  ...
  it("counts owned tasks on key results", () => {
    const indicator = createIndicator("user-2");
    indicator.tasks = [createTask("user-1"), createTask("user-2")];
...
describe("checkUserDeletion()", () => {
  ...
  it("warns about owned missions", () => {
    const missions = [
      createMission({ ownerId: "user-1", status: "completed" }),
    ];
    const result = checkUserDeletion(missions, "user-1", false, false);
  ...
  it("warns about owned key results", () => {
    const missions = [
      createMission({
        ownerId: "user-2",
        indicators: [createIndicator("user-1", "completed")],
      }),
    ];
    const result = checkUserDeletion(missions, "user-1", false, false);
  ...
  it("blocks deletion for active key results", () => {
    const missions = [
      createMission({
        ownerId: "user-2",
        indicators: [createIndicator("user-1", "on_track")],
      }),
    ];
    const result = checkUserDeletion(missions, "user-1", false, false);
...
describe("transferUserOwnership()", () => {
  ...
  it("transfers key result ownership", () => {
    const missions = [
      createMission({
        id: "m1",
        ownerId: "user-2",
        indicators: [createIndicator("user-1"), createIndicator("user-2")],
      }),
    ];
    ...
    expect(updated[0]?.indicators?.[0]?.ownerId).toBe("user-3");
    expect(updated[0]?.indicators?.[1]?.ownerId).toBe("user-2");
    expect(transferred.indicators).toBe(1);
  });
  ...
  it("returns zero counts when nothing to transfer", () => {
    ...
    expect(transferred.indicators).toBe(0);
