...
import type { WizardQuestion } from "@/types/survey/survey";
import type { SurveySubmissionRecord } from "@/lib/tempStorage/surveys-store";
...
function aggregateRanking(
  ...
  for (const id of Object.keys(optionMap)) {
    avgRanks[id] = {
      label: optionMap[id] ?? id,
      avgRank:
        rankCounts[id] && rankSums[id] !== undefined
          ? rankSums[id] / rankCounts[id]
          : Infinity,
    };
  }
...
export function aggregateQuestion(
  ...
  switch (question.type) {
    ...
    case "text_short":
    case "text_long": {
      const responses = rawAnswers.filter(
