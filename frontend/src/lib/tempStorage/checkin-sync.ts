import type { LocalCheckIn } from "@/lib/tempStorage/missions-store";
...
export async function syncCheckInOutboxOperation(
  ...
  if (!response.ok) {
    ...
    const message =
      responseText.trim().length > 0 ? responseText : `HTTP ${response.status}`;
