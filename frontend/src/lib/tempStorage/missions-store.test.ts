...
describe("Missions Store", () => {
  ...
  describe("loadMissionsSnapshot()", () => {
    ...
    it("migrates legacy checkInHistory to normalized format", () => {
      ...
      expect(snapshot.checkInIdsByKr["indicator-1"]).toContain("checkin-1");
    });
  });
  ...
  describe("saveMissionsSnapshot()", () => {
    it("saves data to localStorage", () => {
      ...
      expect(
        parsed.missions.some((m: { id: string }) => m.id === "new-mission"),
      ).toBe(true);
    });
  ...
  describe("snapshotToCheckInHistory()", () => {
    it("converts normalized check-ins to history format", () => {
      const snapshot: MissionsStoreSnapshot = {
        ...
        checkInIdsByKr: {
          "indicator-1": ["checkin-2", "checkin-1"],
        },
        checkInOutbox: [],
      };
      ...
      expect(history["indicator-1"]).toBeDefined();
      expect(history["indicator-1"]?.length).toBe(2);
      expect(history["indicator-1"]?.[0]?.id).toBe("checkin-2");
      expect(history["indicator-1"]?.[1]?.id).toBe("checkin-1");
    });
    ...
    it("excludes deleted check-ins", () => {
      ...
      expect(history["indicator-1"]?.length).toBe(1);
      expect(history["indicator-1"]?.[0]?.id).toBe("checkin-1");
    });
    ...
    it("strips internal fields from check-ins", () => {
      const snapshot: MissionsStoreSnapshot = {
        ...
        checkInIdsByKr: {
          "indicator-1": ["checkin-1"],
        },
        checkInOutbox: [],
      };

      const history = snapshotToCheckInHistory(snapshot);
      const checkIn = history["indicator-1"]?.[0];
...
describe("Seed Data Validation", () => {
  ...
  it("seed missions have key results", () => {
    ...
    const missionWithKRs = snapshot.missions.find(
      (m) => m.indicators && m.indicators.length > 0,
    );

    expect(missionWithKRs).toBeDefined();
    expect(missionWithKRs?.indicators?.[0]?.id).toBeDefined();
  });
  ...
  it("seed check-ins have proper structure", () => {
    ...
    expect(checkIn?.indicatorId).toBeDefined();
