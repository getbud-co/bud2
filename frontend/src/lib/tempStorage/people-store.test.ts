...
describe("Utility Functions", () => {
  describe("createRoleIdForOrg()", () => {
    ...
    it("handles different org and role combinations", () => {
      expect(createRoleIdForOrg("org-2", "admin-rh")).toBe(
        "role-org-2-admin-rh",
      );
      expect(createRoleIdForOrg("brq", "super-admin")).toBe(
        "role-brq-super-admin",
      );
    });
  });
  ...
  describe("createTeamIdFromName()", () => {
    ...
    it("converts to lowercase and replaces spaces", () => {
      expect(createTeamIdFromName("Product Design")).toBe(
        "team-product-design",
      );
    });
...
describe("Data Sanitization", () => {
  ...
  it("filters out team members referencing non-existent users/teams", () => {
    ...
    const invalidMembers = loaded.teamMembers.filter(
      (m) =>
        m.teamId === "non-existent-team" || m.userId === "non-existent-user",
    );
  ...
  it("defaults role to colaborador when missing roleType", () => {
    ...
    expect(snapshot.usersById["test-user"]?.roleId).toBe(
      "role-org-1-colaborador",
    );
  });
});
