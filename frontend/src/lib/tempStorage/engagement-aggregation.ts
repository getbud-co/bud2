...
import type { UserActivity } from "@/types/activity/activity";
import type {
  AlertLevel,
  HealthStatus,
  TrendDirection,
...
import {
  ...
  getPendingSurveyIds,
  getPulseInsights,
} from "@/lib/tempStorage/activity-store";
import { today, addWeeks, startOfWeek } from "@/lib/tempStorage/seed-utils";
...
export function calculateHabitMetrics(
  ...
  const completedSurveys = countSurveysCompletedForUser(
    activities,
    userId,
    teamSurveyIds,
  );
  ...
  const pendingSurveyIds = getPendingSurveyIds(
    activities,
    userId,
    teamSurveyIds,
  );
...
export function calculatePerformanceMetrics(
  ...
  // Indicators ativos do usuário
  const activeIndicators = userMissions
    .filter((m) => m.status === "active" || m.status === "paused")
    .flatMap((m) => m.indicators ?? [])
    .filter((indicator) => indicator.owner?.id === userId);

  const completedIndicators = activeIndicators.filter(
    (indicator) => indicator.progress >= 100,
  );
  const activeIndicatorsCount = activeIndicators.length;
  const completedIndicatorsCount = completedIndicators.length;

  const indicatorsCompletedRate =
    activeIndicatorsCount > 0
      ? completedIndicatorsCount / activeIndicatorsCount
      : 0;
  ...
  const avgProgress =
    activeMissions.length > 0
      ? Math.round(
          activeMissions.reduce((s, m) => s + m.progress, 0) /
  ...
  const avgSurveyResponseTimeHours = calcAvgSurveyResponseTime(
    activities,
    userId,
  );

  return {
    indicatorsCompletedRate,
    avgProgress,
    avgConfidence,
...
export function calculateEngagementScore(habit: UserHabitMetrics): number {
  ...
  const streakScore = Math.min(
    100,
    Math.round((habit.checkInStreak / 4) * 100),
  );
  const freqScore = Math.min(
    100,
    Math.round((habit.checkInsLast30d / 4) * 100),
  );
  ...
  // Acesso: 20+ dias = 100, proporcional
  const accessScore = Math.min(
    100,
    Math.round((habit.activeDays30d / 20) * 100),
  );

  const score = Math.round(
    checkInScore * 0.5 + surveyScore * 0.3 + accessScore * 0.2,
  );
 ...
 *
 * Pesos:
 * - Missões (Indicators completados + progresso + confiança): 60%
...
export function calculatePerformanceScore(
  ...
  // Indicators completados: linear 0-100
  const indicatorsScore = Math.round(perf.indicatorsCompletedRate * 100);
  ...
  const confidenceScore =
    perf.avgConfidence === "high"
      ? 100
      : perf.avgConfidence === "medium"
        ? 60
        : perf.avgConfidence === "low"
          ? 20
          : 50; // sem dados = neutro

  // Score de missões: sem Indicators usa só confiança como proxy
  const missionScore =
    perf.activeIndicatorsCount === 0
      ? Math.round(confidenceScore * 0.5 + 50 * 0.5)
      : Math.round(
          indicatorsScore * 0.4 + progressScore * 0.4 + confidenceScore * 0.2,
        );

  // Score de pesquisas: participação linear 0-100
  const surveyScore = Math.round(surveyParticipation * 100);

  const score = Math.round(missionScore * 0.6 + surveyScore * 0.4);
  return Math.max(0, Math.min(100, score));
}
...
export function determineHealthStatus(overallScore: number): HealthStatus {
  ...
  if (overallScore >= 40) return "attention";
  return "critical";
}
...
export function buildAlertMessages(
  ...
  if (performance.avgProgress < 30 && performance.activeIndicatorsCount > 0) {
...
export function buildMemberEngagementSummary(
  ...
  const habit = calculateHabitMetrics(
    userId,
    checkInHistory,
    activities,
    teamSurveyIds,
  ...
  const performanceScore = calculatePerformanceScore(
    performance,
    habit.surveyResponseRate,
  );
  const overallScore = Math.round(
    engagementScore * 0.5 + performanceScore * 0.5,
  );

  const alertLevel = determineAlertLevel(
    engagementScore,
    performanceScore,
    habit.pendingSurveysCount,
  );
...
export function aggregateTeamEngagement(
  ...
  const checkInRateThisWeek = calculateTeamCheckInRateThisWeek(
    memberIds,
    checkInHistory,
  );
  ...
  const membersInAlert = members.filter(
    (m) => m.alertLevel === "critical",
  ).length;
  ...
  const engagementTrend: TrendDirection =
    upEngagement > downEngagement
      ? "up"
      : downEngagement > upEngagement
        ? "down"
        : "stable";
