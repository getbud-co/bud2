...
function buildPermissions(): Permission[] {
  return [
    createPermission({
      ...
      group: "people",
      label: "Visualizar",
      description: "Ver informações de colaboradores",
    }),
    createPermission({
      id: "people.create",
      group: "people",
    ...
    createPermission({
      id: "people.deactivate",
      group: "people",
      label: "Desativar",
      description: "Desativar contas de colaboradores",
    }),
    createPermission({
      id: "missions.view",
      group: "missions",
      label: "Visualizar",
      description: "Ver missões e OKRs",
    }),
    createPermission({
      id: "missions.create",
      group: "missions",
      label: "Criar",
      description: "Criar novas missões e OKRs",
    }),
    createPermission({
      id: "missions.edit",
      group: "missions",
      label: "Editar",
      description: "Alterar missões e OKRs existentes",
    }),
    createPermission({
      id: "missions.delete",
      group: "missions",
      label: "Excluir",
      description: "Remover missões e OKRs",
    }),
    createPermission({
      id: "missions.assign",
      group: "missions",
      label: "Atribuir",
      description: "Atribuir missões a colaboradores",
    }),
    createPermission({
      id: "surveys.view",
      group: "surveys",
      label: "Visualizar",
      description: "Ver pesquisas disponíveis",
    }),
    createPermission({
      id: "surveys.create",
      group: "surveys",
      label: "Criar",
      description: "Criar novas pesquisas",
    }),
    createPermission({
      id: "surveys.edit",
      group: "surveys",
      label: "Editar",
      description: "Alterar pesquisas existentes",
    }),
    createPermission({
      id: "surveys.publish",
      group: "surveys",
      label: "Publicar",
      description: "Publicar pesquisas para respondentes",
    }),
    createPermission({
      id: "surveys.results",
      group: "surveys",
      label: "Ver resultados",
      description: "Acessar resultados e análises",
    }),
    createPermission({
      id: "settings.access",
      group: "settings",
    ...
    createPermission({
      ...
      group: "settings",
      label: "Editar",
      description: "Alterar configurações da plataforma",
    }),
  ];
}
...
function buildOrgDefaults(orgId: string): {
  ...
  const values = [
    createCompanyValue(orgId, {
      ...
      name: "Inovação",
      description:
        "Buscamos soluções criativas e desafiamos o status quo em tudo que fazemos.",
    }),
    createCompanyValue(orgId, {
      id: "value-colaboracao",
      name: "Colaboração",
      description:
        "Acreditamos que resultados extraordinários vêm do trabalho em equipe.",
    }),
    createCompanyValue(orgId, {
      id: "value-transparencia",
      name: "Transparência",
      description:
        "Comunicamos com honestidade e clareza, construindo confiança mútua.",
    }),
    createCompanyValue(orgId, {
    ...
    createCompanyValue(orgId, {
      ...
      name: "Excelência",
      description:
        "Entregamos com qualidade e buscamos melhorar continuamente.",
    }),
    createCompanyValue(orgId, {
      id: "value-integridade",
      name: "Integridade",
      description:
        "Agimos com ética, responsabilidade e coerência em todas as relações.",
    }),
  ];
  ...
  const allPermissionIds = buildPermissions().map(
  ...
  const roles = [
    ...
    createRole(orgId, {
      ...
      isDefault: false,
      scope: "org",
      permissionIds: allPermissionIds.filter(
        (permissionId) => permissionId !== "settings.edit",
      ),
    }),
    createRole(orgId, {
      slug: "gestor",
    ...
    createRole(orgId, {
      ...
      isDefault: true,
      scope: "self",
      permissionIds: ["people.view", "missions.view", "surveys.view"],
    }),
    createRole(orgId, {
      slug: "visualizador",
      name: "Visualizador",
...
export function getSeedSnapshot(): ConfigStoreSnapshot {
  // Example hierarchy:
  // - BRQ (holding)
  ...
  const organizations = [
    ...
    createOrganization({
      ...
      logoUrl:
        "https://ui-avatars.com/api/?name=IT&background=0EA5E9&color=fff&size=48&font-size=0.4&bold=true",
      plan: "starter",
      orgType: "company",
    }),
  ];
...
function sanitizeOrganization(raw: unknown): CompanyProfile | null {
  ...
  const orgType =
    (item.orgType as OrgType) ?? (parentOrgId ? "subsidiary" : "company");
  const depth =
    typeof item.depth === "number" ? item.depth : parentOrgId ? 1 : 0;
...
function sanitizeCycle(raw: unknown, orgId: string): Cycle | null {
  ...
  return {
    ...
    okrDefinitionDeadline: item.okrDefinitionDeadline
      ? String(item.okrDefinitionDeadline)
      : null,
...
function migrateSnapshot(
  raw: Partial<ConfigStoreSnapshot> | null,
): ConfigStoreSnapshot {
  ...
  Object.keys(organizationsById).forEach((orgId) => {
    ...
    companyValuesByOrg[orgId] =
      nextValues.length === 0 ? seedValues : [...nextValues, ...newSeedValues];
    ...
    tagsByOrg[orgId] =
      nextTags.length === 0 ? seedTags : [...nextTags, ...newSeedTags];
  ...
  const activeOrgId =
    raw.activeOrgId && organizationsById[raw.activeOrgId]
      ? raw.activeOrgId
  ...
  return {
    ...
    legacyRoleSlugAliases: mergeAliases(
      DEFAULT_ROLE_ALIASES,
      raw.legacyRoleSlugAliases,
    ),
    legacyCycleIdAliases: mergeAliases(
      DEFAULT_CYCLE_ALIASES,
      raw.legacyCycleIdAliases,
    ),
    legacyTagIdAliases: mergeAliases(
      DEFAULT_TAG_ALIASES,
      raw.legacyTagIdAliases,
    ),
  };
}
...
export function saveConfigSnapshot(
  snapshot: Omit<ConfigStoreSnapshot, "schemaVersion" | "updatedAt">,
): ConfigStoreSnapshot {
