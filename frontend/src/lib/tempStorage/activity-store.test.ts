...
describe("getInitialActivitySnapshot", () => {
  it("retorna snapshot com versão 1", () => {
  ...
  it("inclui atividades de check-in", () => {
    const snapshot = getInitialActivitySnapshot();
    const checkIns = snapshot.activities.filter(
      (a) => a.type === "checkin_create",
    );
  ...
  it("inclui atividades de survey", () => {
    const snapshot = getInitialActivitySnapshot();
    const surveys = snapshot.activities.filter(
      (a) => a.type === "survey_complete",
    );
...
describe("countActiveDaysForUser", () => {
  ...
  it("ignora atividades que não são login", () => {
    const activities = [makeActivity("u1", "checkin_create", 1)];
...
describe("countSurveysCompletedForUser", () => {
  it("conta surveys completadas pelo usuário", () => {
    const activities = [
      ...
      makeActivity("u1", "survey_start", 4, {
        entityId: "s3",
        entityType: "survey",
      }), // Não completada
    ];
    expect(
      countSurveysCompletedForUser(activities, "u1", ["s1", "s2", "s3"]),
    ).toBe(2);
  });
...
describe("calcAvgSurveyResponseTime", () => {
  it("calcula média de tempo de resposta", () => {
    const activities = [
      makeActivity("u1", "survey_complete", 5, {
        metadata: { responseTimeHours: 12 },
      }),
      makeActivity("u1", "survey_complete", 3, {
        metadata: { responseTimeHours: 24 },
      }),
    ];
