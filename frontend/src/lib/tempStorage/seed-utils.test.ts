...
describe("Check-in Generation", () => {
  ...
  describe("generateCheckInsForIndicator()", () => {
    it("generates requested number of check-ins", () => {
      const checkIns = generateCheckInsForIndicator("indicator-test-1", 4);
      expect(checkIns.length).toBeLessThanOrEqual(4);
    });

    it("returns deterministic results for same ID", () => {
      const checkIns1 = generateCheckInsForIndicator("indicator-test-123");
      const checkIns2 = generateCheckInsForIndicator("indicator-test-123");
    ...
    it("returns different results for different IDs", () => {
      const checkIns1 = generateCheckInsForIndicator("indicator-alpha");
      const checkIns2 = generateCheckInsForIndicator("indicator-beta");
    ...
    it("all check-ins are in the past", () => {
      const now = new Date();
      const checkIns = generateCheckInsForIndicator("indicator-test");
    ...
    it("check-ins are sorted by date descending", () => {
      const checkIns = generateCheckInsForIndicator("indicator-test", 4);
