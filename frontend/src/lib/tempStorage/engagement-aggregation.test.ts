...
import type { UserEngagementSummary } from "@/types/engagement/engagement";
import { today, addDays, toIsoDateTime } from "@/lib/tempStorage/seed-utils";
...
function makeMemberEngagement(
  overrides: Partial<UserEngagementSummary> = {},
  ...
  return {
    ...
    performance: {
      indicatorsCompletedRate: 0.75,
      avgProgress: 70,
      avgConfidence: "high",
      activeIndicatorsCount: 4,
      completedIndicatorsCount: 3,
...
describe("calculateHabitMetrics", () => {
  ...
  it("calcula surveyResponseRate corretamente", () => {
    const activities = [
      makeActivity("u1", "survey_complete", 5, {
        entityId: "s1",
        entityType: "survey",
      }),
    ];
  ...
  it("pendingSurveysCount = 0 quando respondeu todas", () => {
    const activities = [
      ...
      makeActivity("u1", "survey_complete", 3, {
        entityId: "s2",
        entityType: "survey",
      }),
    ];
  ...
  it("retorna daysSinceLastCheckIn correto", () => {
    const history = { "indicator-1": [makeCheckIn("u1", 3)] };
...
describe("calculateAvgConfidence", () => {
  ...
  it("retorna low quando maioria é low", () => {
    const checkIns = [makeCheckIn("u1", 1, "low"), makeCheckIn("u1", 2, "low")];
...
describe("calculatePerformanceScore", () => {
  it("retorna score entre 0 e 100", () => {
    const perf = {
      indicatorsCompletedRate: 0.5,
      avgProgress: 60,
      avgConfidence: "medium" as const,
      activeIndicatorsCount: 4,
      completedIndicatorsCount: 2,
  ...
  it("retorna score neutro quando sem Indicators ativos", () => {
    const perf = {
      indicatorsCompletedRate: 0,
      avgProgress: 0,
      avgConfidence: null,
      activeIndicatorsCount: 0,
      completedIndicatorsCount: 0,
...
describe("determineHealthStatus", () => {
  it("healthy quando overall >= 70", () => {
    expect(determineHealthStatus(70)).toBe("healthy");
    expect(determineHealthStatus(85)).toBe("healthy");
    expect(determineHealthStatus(100)).toBe("healthy");
  });

  it("attention quando overall entre 40 e 69", () => {
    expect(determineHealthStatus(40)).toBe("attention");
    expect(determineHealthStatus(55)).toBe("attention");
    expect(determineHealthStatus(69)).toBe("attention");
  });

  it("critical quando overall < 40", () => {
...
describe("buildAlertMessages", () => {
  ...
  it("gera alerta para muitos dias sem check-in", () => {
    const alerts = buildAlertMessages(
      { ...baseHabit, daysSinceLastCheckIn: 10 },
      basePerf,
    );
  ...
  it("gera alerta para pesquisas pendentes", () => {
    const alerts = buildAlertMessages(
      { ...baseHabit, pendingSurveysCount: 2 },
      basePerf,
    );
  ...
  it("gera alerta para progresso baixo", () => {
    const alerts = buildAlertMessages(baseHabit, {
      ...basePerf,
      avgProgress: 15,
    });
...
describe("buildMemberEngagementSummary", () => {
  ...
  it("overallScore é média 50/50 de engagement e performance", () => {
    ...
    const expected = Math.round(
      result.engagementScore * 0.5 + result.performanceScore * 0.5,
    );
...
describe("aggregateTeamEngagement", () => {
  it("retorna zeros para time vazio", () => {
    ...
    expect(result.byHealthStatus).toEqual({
      healthy: 0,
      attention: 0,
      critical: 0,
    });
