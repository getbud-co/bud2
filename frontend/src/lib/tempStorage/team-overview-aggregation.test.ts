...
import type { Mission } from "@/types/mission/mission";
...
function makeSurvey(
  overrides: Partial<SurveyListItemData> = {},
): SurveyListItemData {
  return {
    id: "s1",
    templateId: null,
...
describe("aggregateTeamMissions", () => {
  ...
  it("marca missão como atRisk quando progresso baixo e prazo próximo", () => {
    ...
    const missions = [
      makeMission({ progress: 20, dueDate: due, status: "active" }),
    ];
    const result = aggregateTeamMissions(missions, "team-1", []);
  ...
  it("não marca como atRisk missão com progresso >= 70", () => {
    ...
    const missions = [
      makeMission({ progress: 75, dueDate: due, status: "active" }),
    ];
    const result = aggregateTeamMissions(missions, "team-1", []);
...
describe("aggregateTeamSurveys", () => {
  ...
  it("exclui surveys arquivadas", () => {
    const surveys = [
      makeSurvey({ status: "archived", managerIds: ["user-1"] }),
    ];
    const result = aggregateTeamSurveys(surveys, ["user-1"]);
