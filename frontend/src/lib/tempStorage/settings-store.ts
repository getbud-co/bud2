...
export const TONE_PRESETS: TonePreset[] = [
  {
    ...
    label: "Profissional",
    description:
      "Comunicação clara e formal, ideal para contextos corporativos tradicionais",
    example:
      "Identifiquei que o engajamento do time de Design caiu 12% neste mês. Recomendo agendar um check-in com a equipe para investigar possíveis causas.",
    iconName: "Handshake",
  },
  {
    id: "direto",
    ...
    description:
      "Vai direto ao ponto, sem rodeios. Para gestores que valorizam eficiência",
    example:
      "Engajamento de Design caiu 12%. Ação sugerida: check-in com o time esta semana.",
    iconName: "Target",
  },
  {
    id: "empatico",
    ...
    description:
      "Tom humano e cuidadoso, bom para culturas que priorizam bem-estar",
    example:
      "Notei que o time de Design pode estar passando por um momento desafiador — o engajamento caiu um pouco este mês. Que tal conversar com eles para entender como estão se sentindo?",
    iconName: "ChatCircle",
  },
  {
    id: "coach",
    label: "Coach",
    description:
      "Faz perguntas ao invés de dar respostas, estimulando reflexão do gestor",
    example:
      "O engajamento do Design caiu 12%. O que você acha que pode ter mudado recentemente? Existe algum fator que vale investigar com o time?",
    iconName: "Brain",
  },
  {
    id: "energetico",
    ...
    description:
      "Tom positivo e encorajador, que celebra conquistas e inspira ação",
    example:
      "Opa, o engajamento do Design merece atenção — caiu 12%, mas nada que uma boa conversa não resolva! Que tal agendar um papo com o time?",
    iconName: "Lightning",
  },
];
...
export const DATA_SOURCE_CATALOG: DataSourceCatalogItem[] = [
  {
    ...
    name: "Notion",
    description:
      "Acesse páginas, bases de dados e documentos do Notion como contexto para a IA.",
  ...
  },
  {
    ...
    name: "Confluence",
    description:
      "Conecte sua wiki do Confluence para que a IA acesse políticas, processos e documentação interna.",
  ...
  },
  {
    ...
    name: "Google Drive",
    description:
      "Acesse documentos, planilhas e apresentações do Google Drive como referência para a IA.",
  ...
  },
  {
    ...
    name: "SharePoint",
    description:
      "Conecte o SharePoint para acessar documentos e políticas organizacionais.",
  ...
  },
  {
    ...
    name: "Jira",
    description:
      "Atualize o progresso de OKRs automaticamente quando tarefas são concluídas no Jira.",
  ...
  },
  {
    ...
    name: "Google Sheets",
    description:
      "Importe e exporte dados de performance, OKRs e relatórios para planilhas.",
  ...
  },
  {
    ...
    description:
      "Conecte qualquer API REST compatível para estender as capacidades da IA.",
    tools: ["Customizável"],
    requiresPro: true,
  },
];
...
function createDefaultAiSettings(orgId: string): OrgAiSettings {
  ...
  return {
    ...
    customInstructions:
      "Use linguagem acessível e evite jargões técnicos. Sempre sugira próximos passos concretos ao final de cada análise.",
...
function createSeedKnowledgeDocs(orgId: string): KnowledgeDocRecord[] {
  ...
  return [
    ...
    },
    {
      ...
      size: null,
      content:
        "A ACME LTDA é uma empresa de tecnologia com 350 colaboradores, fundada em 2015. Valores: inovação, transparência, colaboração.",
      addedAt: doc3Date,
    },
  ];
}
...
function sanitizeAiSettings(raw: unknown, orgId: string): OrgAiSettings | null {
  ...
  const validProactivity: ProactivityLevel[] = [
    "minimum",
    "moderate",
    "default",
    "maximum",
  ...
  return {
    orgId,
    toneId: validTones.includes(item.toneId as ToneId)
      ? (item.toneId as ToneId)
      : defaults.toneId,
    language:
      typeof item.language === "string" ? item.language : defaults.language,
    respectWorkingHours:
      typeof item.respectWorkingHours === "boolean"
        ? item.respectWorkingHours
        : defaults.respectWorkingHours,
    workingHoursStart:
      typeof item.workingHoursStart === "string"
        ? item.workingHoursStart
        : defaults.workingHoursStart,
    workingHoursEnd:
      typeof item.workingHoursEnd === "string"
        ? item.workingHoursEnd
        : defaults.workingHoursEnd,
    proactivityLevel: validProactivity.includes(
    ...
    suggestionTypes: {
      oneOnOnePrep:
        typeof item.suggestionTypes?.oneOnOnePrep === "boolean"
          ? item.suggestionTypes.oneOnOnePrep
          : defaults.suggestionTypes.oneOnOnePrep,
      coachingTips:
        typeof item.suggestionTypes?.coachingTips === "boolean"
          ? item.suggestionTypes.coachingTips
          : defaults.suggestionTypes.coachingTips,
      teamAlerts:
        typeof item.suggestionTypes?.teamAlerts === "boolean"
          ? item.suggestionTypes.teamAlerts
          : defaults.suggestionTypes.teamAlerts,
      reviewDrafts:
        typeof item.suggestionTypes?.reviewDrafts === "boolean"
          ? item.suggestionTypes.reviewDrafts
          : defaults.suggestionTypes.reviewDrafts,
      okrSuggestions:
        typeof item.suggestionTypes?.okrSuggestions === "boolean"
          ? item.suggestionTypes.okrSuggestions
          : defaults.suggestionTypes.okrSuggestions,
      dailyBriefing:
        typeof item.suggestionTypes?.dailyBriefing === "boolean"
          ? item.suggestionTypes.dailyBriefing
          : defaults.suggestionTypes.dailyBriefing,
    },
    transparencyMode: validTransparency.includes(
    ...
    biasDetectionEnabled:
      typeof item.biasDetectionEnabled === "boolean"
        ? item.biasDetectionEnabled
        : defaults.biasDetectionEnabled,
    dataSharingEnabled:
      typeof item.dataSharingEnabled === "boolean"
        ? item.dataSharingEnabled
        : defaults.dataSharingEnabled,
    monthlyUsageLimit:
      typeof item.monthlyUsageLimit === "number"
        ? item.monthlyUsageLimit
        : null,
...
function sanitizeDataSource(
  ...
  const validStatuses: DataSourceStatus[] = [
    "connected",
    "disconnected",
    "error",
  ];
  ...
  return {
    id: String(item.id),
    orgId,
    type: validTypes.includes(item.type as DataSourceType)
      ? (item.type as DataSourceType)
      : "custom",
    name: String(item.name),
    description: typeof item.description === "string" ? item.description : "",
    status: validStatuses.includes(item.status as DataSourceStatus)
      ? (item.status as DataSourceStatus)
      : "disconnected",
    enabled: typeof item.enabled === "boolean" ? item.enabled : false,
    url: typeof item.url === "string" ? item.url : "",
    lastSync: typeof item.lastSync === "string" ? item.lastSync : null,
    accessSummary:
      typeof item.accessSummary === "string" ? item.accessSummary : null,
...
function sanitizeKnowledgeDoc(
  ...
  return {
    ...
    orgId,
    name: String(item.name),
    type: validTypes.includes(item.type as KnowledgeDocType)
      ? (item.type as KnowledgeDocType)
      : "text",
...
function migrateSnapshot(
  raw: Partial<SettingsStoreSnapshot> | null,
  ...
  if (!dataSourcesByOrg[DEFAULT_ORG_ID]) {
    dataSourcesByOrg[DEFAULT_ORG_ID] =
      seed.dataSourcesByOrg[DEFAULT_ORG_ID] ?? [];
  }
  ...
  if (!knowledgeDocsByOrg[DEFAULT_ORG_ID]) {
    knowledgeDocsByOrg[DEFAULT_ORG_ID] =
      seed.knowledgeDocsByOrg[DEFAULT_ORG_ID] ?? [];
  }
