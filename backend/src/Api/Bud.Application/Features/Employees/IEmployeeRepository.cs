...
public interface IEmployeeRepository
{
    ...
    Task<Employee?> GetByIdWithEmployeeTeamsAsync(Guid id, CancellationToken ct = default);
    Task<PagedResult<Employee>> GetAllAsync(Guid? teamId, string? search, int page, int pageSize, CancellationToken ct = default);
    Task<List<Employee>> GetLeadersAsync(Guid? organizationId, CancellationToken ct = default);
