...
public sealed class CloudLoggingJsonFormatter : ConsoleFormatter
{
        ...
        TextWriter textWriter)
    {
        ...
        // severity: mapped from LogLevel to GCP severity strings
        ...
        // logging.googleapis.com/sourceLocation: category + eventId
        ...
        // Additional scope properties (key/value pairs set via BeginScope)
        if (options.IncludeScopes && scopeProvider is not null)
        {
            scopeProvider.ForEachScope((scope, w) =>
            {
                if (scope is IReadOnlyList<KeyValuePair<string, object?>> kvList)
                {
                    ...
                    {
                        // Skip trace-related fields already written above
    ...
    // Minimal pooled buffer writer for Utf8JsonWriter
