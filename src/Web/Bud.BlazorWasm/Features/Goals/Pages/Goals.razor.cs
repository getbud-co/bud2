...
namespace Bud.BlazorWasm.Features.Goals.Pages;
...
public partial class Goals
{
    ...
    private async Task HandleOrganizationChangedAsync()
    {
        ...
        {
            Console.Error.WriteLine($"Erro ao atualizar metas por troca de organização: {ex.Message}");
            ToastService.ShowError("Erro ao atualizar metas", "Não foi possível atualizar os dados da organização selecionada.");
        }
    }
    ...
    private async Task HandleWizardSave(GoalFormResult result)
    {
        ...
        else
        {
            await CreateGoalFromResult(result, GoalStatus.Active,
                ...
                m => $"A meta '{m?.Name}' foi criada e está ativa.",
                "Erro ao criar meta",
    ...
    private async Task HandleWizardSaveDraft(GoalFormResult result)
    {
        await CreateGoalFromResult(result, GoalStatus.Planned,
            ...
            _ => "A meta foi salva como rascunho.",
            "Erro ao salvar rascunho",
            "Não foi possível salvar o rascunho da meta. Verifique os dados e tente novamente.");
    }
    ...
    private async Task UpdateGoalFromResult(Guid goalId, GoalFormResult result)
    {
        await UiOps.RunAsync(
            async () =>
            {
                ...
                ToastService.ShowSuccess("Meta atualizada", "As alterações foram salvas com sucesso.");
                CloseWizard();
            },
            "Erro ao atualizar",
    ...
    private async Task DeleteGoal(Guid goalId)
    {
        try
        {
            await UiOps.RunAsync(
                ...
                {
                    await Api.DeleteGoalAsync(goalId);
                    ToastService.ShowSuccess("Meta excluída", "A meta foi removida com sucesso.");
                    await LoadGoals();
                },
            "Erro ao excluir",
